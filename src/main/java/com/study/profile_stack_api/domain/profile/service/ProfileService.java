...
public class ProfileService {
    ...
    public ProfileResponse getProfileById(Long id) {
        // 검증
        if (id == null || id <= 0) {
            throw new BusinessException(ErrorCode.INVALID_INPUT, "잘못된 id 값입니다.");
        }
    ...
    private void validateCreateProfile(ProfileCreateRequest request) {
        ...
        if (request.getName() == null) {
            throw new BusinessException(ErrorCode.INVALID_INPUT, "이름은 필수 입력입니다.");
        } else {
        ...
        if (request.getEmail() == null) {
            throw new BusinessException(ErrorCode.INVALID_INPUT, "이메일은 필수 입력입니다.");
        } else {
    ...
    private void validateUpdateProfile(ProfileUpdateRequest request) {
        if (request.hasNoUpdates()) {
            throw new BusinessException(ErrorCode.INVALID_INPUT, "수정할 목록이 없습니다.");
        }
    ...
    private void validateName(String name) {
        if (name.trim().isEmpty()) {
            throw new BusinessException(ErrorCode.INVALID_INPUT, "이름은 입력으로 빈 문자열을 가질 수 없습니다.");
        }

        if (name.length() > 50)
            throw new BusinessException(ErrorCode.INVALID_INPUT, "이름은 1자 이상, 50자 이하여야 합니다.");

    }

    private void validateEmail(String email, boolean checkExisted) {
        if (email.trim().isEmpty()) {
            throw new BusinessException(ErrorCode.INVALID_INPUT, "이메일은 입력으로 빈 문자열을 가질 수 없습니다.");
        }
        if (email.length() > 100) {
            throw new BusinessException(ErrorCode.INVALID_INPUT, "이메일은 1자 이상, 100자 이하여야 합니다.");
        }
        if (checkExisted && repository.existsByEmail(email)) {
            throw new BusinessException(ErrorCode.DUPLICATE_EMAIL, "이미 존재하는 이메일 주소입니다.");
        }
    }

    private void validateBio(String bio) {
        if (bio != null && bio.length() > 500) {
            throw new BusinessException(ErrorCode.INVALID_INPUT, "자기소개는 500자 이하여야 합니다.");
        }
    }

    private void validatePosition(String position) {
        if (position.trim().isEmpty()) {
            throw new BusinessException(ErrorCode.INVALID_INPUT, "포지션은 입력으로 빈 문자열을 가질 수 없습니다.");
        }
        try {
            Position.valueOf(position);
        } catch (IllegalArgumentException e) {
            throw new BusinessException(ErrorCode.INVALID_INPUT, "정의되지 않은 직무 입력입니다.");
        }
    }

    private void validateCareerYears(Integer careerYears) {
        if (careerYears < 0) {
            throw new BusinessException(ErrorCode.INVALID_INPUT, "경력 연차는 0 이상이어야 합니다.");
        }
    }

    private void validateGithubUrl(String githubUrl) {
        if (githubUrl.trim().isEmpty()) {
            throw new BusinessException(ErrorCode.INVALID_INPUT, "깃허브 URL은 입력으로 빈 문자열을 가질 수 업습니다.");
        }
    }

    private void validateBlogUrl(String blogUrl) {
        if (blogUrl.trim().isEmpty()) {
            throw new BusinessException(ErrorCode.INVALID_INPUT, "블로그 URL은 입력으로 빈 문자열을 가질 수 업습니다.");
        }
    }
}
